...
for name, info in car_data.items():
    year_ok   = info[1] >= search_criteria[0]
    engine_ok = info[2] >= search_criteria[1]
    price_ok  = info[4] <= search_criteria[2]
    if year_ok and engine_ok and price_ok:
        results.append((name, info))

results = sorted(results, key=lambda x: x[1][4])[:5]
